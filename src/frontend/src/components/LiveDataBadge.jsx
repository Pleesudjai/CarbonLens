...
export default function LiveDataBadge({ loading = false, error = null, sourceSummary = '' }) {
  ...
  if (loading) {
    return (
      <div className="absolute right-4 bottom-4 z-10 rounded-xl border border-amber-200 bg-amber-50/95 px-3 py-2 shadow-lg backdrop-blur-sm">
  ...
  return (
    <div className="absolute right-4 bottom-4 z-10 rounded-xl border border-emerald-200 bg-white/95 px-3 py-2 shadow-lg backdrop-blur-sm">
      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-700">Data Mode</p>
      <div className="mt-2 flex flex-wrap gap-2">
        {pills.map((pill) => (
          <span
            key={pill}
            className="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[10px] font-semibold text-emerald-700"
          >
            {pill}
          </span>
        ))}
