...
1. **Read global rules:**
   Read `CLAUDE.md` - has the active product direction, stack, APIs, and coding standards.
...
5. **Scan current source:**
   ...
   - `src/backend/analysis/` - transit analysis modules
   - `src/backend/gee/` - legacy area only if relevant to the current spec
